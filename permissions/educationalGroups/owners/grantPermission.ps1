################################################################
# HelloID-Conn-Prov-Target-Microsoft-Entra-ID-EducationalGroups-Owners-GrantPermission
# PowerShell V2
################################################################
        ...
        'GrantPermission' {
            if (-not($actionContext.DryRun -eq $true)) {
                ...
                $splatGrantPermission = @{
                    Uri     = "https://graph.microsoft.com/v1.0/groups/$($actionContext.References.Permission.Id)/owners/`$ref"
                    Headers = $headers
                    Method  = 'POST'
                    Verbose = $false
                    Body    = @{
                ...
                try {
                    ...
                    if (($_.ErrorDetails.Message | ConvertFrom-Json).error.message -ne 'One or more added object references already exist for the following modified properties: ''owners''.') {
