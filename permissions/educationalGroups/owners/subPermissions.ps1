################################################################
# HelloID-Conn-Prov-Target-Microsoft-Entra-ID-EducationalGroups-Owners-SubPermissions
# PowerShell V2
################################################################
...
try {
    ...
    if (-Not($actionContext.Operation -eq "revoke")) {
        ...
        foreach ($contract in $personContext.Person.Contracts) {
            ...
            $lesGroep = $contract.Department.ExternalId   
            $teamNamePrefix = $actionContext.Configuration.classTeamPrefix      
            ...
            $actionMessage = "querying Microsoft Entra ID group for resource: $($teamName | ConvertTo-Json)"
    ...
    foreach ($permission in $currentPermissions.GetEnumerator()) {
        if (-Not $desiredPermissions.ContainsKey($permission.Name) -AND $permission.Name -ne "No permissions defined") {
            # Microsoft docs: https://learn.microsoft.com/en-us/graph/api/group-delete-owners?view=graph-rest-1.0&tabs=http
            $actionMessage = "revoking group [$($permission.Value)] with id [$($permission.Name)] from account"

            $baseUri = "https://graph.microsoft.com/"
            $revokePermissionSplatParams = @{
                Uri         = "$($baseUri)/v1.0/groups/$($permission.Name)/owners/$($actionContext.References.Account)/`$ref"
                Headers     = $headers
                Method      = "DELETE"
                Verbose     = $false
                ErrorAction = "Stop"
            ...
            if (-Not($actionContext.DryRun -eq $true)) {
                try {
                    ...
                    if (($errorObj.ErrorDetails | ConvertFrom-Json).error.code -eq "Request_ResourceNotFound" -and ($errorObj.ErrorDetails | ConvertFrom-Json).error.message -like "*$($actionContext.References.Permission.id)*") {                        
                        $outputContext.AuditLogs.Add([PSCustomObject]@{
                                Action  = "RevokePermission"
                                Message = "Skipped revoking group [$($permission.Value)] with id [$($permission.Name)] from account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Reason: User is already no longer a owner or the group no longer exists."
                                IsError = $false
                            })
                    }
                    else {
    ...
    foreach ($permission in $desiredPermissions.GetEnumerator()) {
        ...
        if (-Not $currentPermissions.ContainsKey($permission.Name)) {
            # Microsoft docs: https://learn.microsoft.com/en-us/graph/api/group-post-owners?view=graph-rest-1.0&tabs=http
            $actionMessage = "granting group [$($permission.Value)] with id [$($permission.Name)] to account"
            
            # Microsoft docs: https://learn.microsoft.com/en-us/graph/api/group-post-owners?view=graph-rest-1.0&tabs=http
            $splatGrantPermission = @{
                Uri     = "https://graph.microsoft.com/v1.0/groups/$($permission.Name)/owners/`$ref"
                Headers = $headers
                Method  = 'POST'
                Verbose = $false
                Body    = @{
            ...
            if (-Not($actionContext.DryRun -eq $true)) {
                try {
                    ...
                    if ($auditMessage -like "*One or more added object references already exist for the following modified properties: 'owners'*") {
                        $outputContext.AuditLogs.Add([PSCustomObject]@{
                                Action  = "GrantPermission"
                                Message = "Skipped granting group [$($permission.Value)] with id [$($permission.Name)] to account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Reason: User is already an owner of the group."
                                IsError = $false
                            })
                    }
                    else {
