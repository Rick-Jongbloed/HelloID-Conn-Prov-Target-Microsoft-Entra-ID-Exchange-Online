#################################################################
# HelloID-Conn-Prov-Target-Microsoft-Entra-ID-EducationalGroups-Owners-RevokePermission
# PowerShell V2
#################################################################
...
try {
    ...
    switch ($action) {
        'RevokePermission' {
            ...
            $revokePermissionSplatParams = @{
                Uri     = "$($baseUri)/v1.0/groups/$($actionContext.References.Permission.Id)/owners/$($actionContext.References.Account)/`$ref"
                Headers = $headers
                Method  = "DELETE"
                Verbose = $false
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        if ($errorObj.ErrorDetails.error.code -eq "Request_ResourceNotFound" -and $errorObj.ErrorDetails.error.message -like "*$($actionContext.References.Permission.id)*") {
            $auditMessage = "Skipped revoking group [$($actionContext.PermissionDisplayName)] with id [$($actionContext.References.Permission.id)] from account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Reason: User is already no longer a owner or the group no longer exists."
            $auditError = $false
            $outputContext.success = $true
        }
        else {
