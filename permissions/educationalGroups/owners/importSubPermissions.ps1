####################################################################
# HelloID-Conn-Prov-Target-Microsoft-Entra-ID-EducationalGroups-Owners-ImportSubPermissions
# PowerShell V2
####################################################################
...
$permissionReference = 'EDU-Teams-Owners'
$permissionDisplayName = 'EDU Teams - Owners'
    ...
    foreach ($entraIDGroup in $entraIDGroups) {  
        ...
        if ($numberOfAccounts -gt 0) {
            ...
            foreach ($batch in $batches) {
                $permission.AccountReferences = [array]($batch.Group | ForEach-Object { @($entraIDGroupOwners[$_].id) })
