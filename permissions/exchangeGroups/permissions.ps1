#####################################################
# HelloID-Conn-Prov-Target-MS-Entra-ExO-Permissions-ExchangeGroups-List
...
function Get-ExODistributionGroups {
    ...
    $Request = @{
        Uri         = "https://outlook.office365.com/adminapi/beta/$TenantID/InvokeCommand?`$select=Guid,DisplayName,RecipientTypeDetails,IsDirSynced"
        Method      = 'Post'
    ...
    $Response.Value |
        ...
        Select-Object Guid, DisplayName, RecipientTypeDetails
}
#endregion Functions
...
try {
    ...
    $Groups = Get-ExODistributionGroups -Authorization $ExOAuthorization -TenantID $ActionContext.Configuration.TenantID
    Write-Information "Retrieved $(($Groups | Measure-Object).Count) Exchange Online groups."

    $Groups | ForEach-Object {
        $displayNamePrefix = if ($_.RecipientTypeDetails -eq 'MailUniversalSecurityGroup') {
        ...
        $displayName = "$displayNamePrefix - $($_.DisplayName)"
